...
def _find_plate_and_file_cols(schema: dict, table: str):
    # Reuse your existing _pick_col helper
    plate_col = _pick_col(
        schema, table,
        candidates=[
    ...
    file_col = _pick_col(
        schema, table,
        candidates=["file_name", "filename", "image_name", "imagefile", "image_path"],
        contains_any=["file", "image", "name", "path"]
    )
    return plate_col, file_col


# 4) Lookup filename by plate (also make plate matching case-insensitive)
def get_filename_for_plate(plate_number: str) -> Optional[str]:
    schema = get_schema(DATASET)
    plate_col, file_col = _find_plate_and_file_cols(schema, TABLE)
    if not plate_col or not file_col:
        raise RuntimeError("Could not find plate or file column in schema.")

    table_fqn = f"`{PROJECT}.{DATASET}.{TABLE}`"
    sql = f"""
    SELECT {file_col} AS filename
    FROM {table_fqn}
    WHERE UPPER({plate_col}) = UPPER(@plate)
    ORDER BY from_time DESC
    LIMIT 1
    """
    job_cfg = bigquery.QueryJobConfig(
        query_parameters=[bigquery.ScalarQueryParameter("plate", "STRING", plate_number)]
    )
    df = bq.query(sql, job_config=job_cfg, location=BQ_LOCATION) \
           .result().to_dataframe(create_bqstorage_client=False)
    if df.empty or not str(df.iloc[0]["filename"]).strip():
        return None
...
# 2) LLM → SQL call (if you use ML.GENERATE_TEXT)
def call_gemini_generate_sql(prompt_text: str) -> str:
    ...
    """
    df = bq.query(sql, job_config=job_config).result().to_dataframe(create_bqstorage_client=False)
    if df.empty:
        return ""
...
# =========================
# 3) Distinct helper (if you have it)
...
# 1) Run arbitrary SQL
def run_sql(sql: str):
    job_cfg = bigquery.QueryJobConfig()  # no 'location' here
    job = bq.query(sql, job_config=job_cfg, location=BQ_LOCATION)  # pass location here
...
for k, v in {
    ...
    "plate_input": "",
    "plate_filename": None,
    "plate_image_bytes": None,         # NEW: changes the widget key on reset
}.items():
...
if reset_clicked:
    ...
    st.session_state["plate_nonce"] += 1
    st.session_state["plate_input"] = ""
    st.session_state["plate_filename"] = None
    st.session_state["plate_image_bytes"] = None
...
st.subheader("Show vehicle image by number plate")

plate_key = f"plate_{st.session_state['plate_nonce']}"

with st.form("show_image_form", clear_on_submit=False):
    plate_input = st.text_input("Vehicle Plate Number", key="plate_input", placeholder="e.g., MH12AB1234")
    show_btn = st.form_submit_button("Show image", type="primary")

if show_btn:
    if not plate_input.strip():
        st.warning("Please enter a plate number.")
    else:
        with st.spinner("Looking up image filename…"):
            try:
                fname = get_filename_for_plate(plate_input.strip())
            except Exception as e:
                st.error(f"Could not retrieve filename: {e}")
                fname = None

        if not fname:
            st.info("No image filename found for that plate.")
        else:
            with st.spinner(f"Downloading image `{fname}` from Cloud Storage…"):
                data, blob_path = download_image_bytes_from_gcs(fname)

            if data:
                st.success(f"Loaded `{blob_path}` from `{BUCKET_NAME}`")
                st.image(data, caption=blob_path, use_container_width=True)
            else:
                st.error(f"Image not found in bucket `{BUCKET_NAME}` for `{fname}`.\n"
                         "Tried common extensions (.jpg/.jpeg/.png).")
